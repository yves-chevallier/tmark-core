...
impl Html<'_> {
    ...
    /// (spec §BlockQuote), where LaTeX writes `\tsepigraph[source={…}]`
    /// and Typst `#ts-epigraph(source: …)`. It is written here rather
    /// than through `attrs`, which prints id, class and lang alone.
